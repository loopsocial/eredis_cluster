...
-export([start/0,repeat/1]).

repeat(0 ) ->
            0;
repeat(N) ->
    erlang:display(N),
    spawn(eredis_cluster_tests, start, []),
    % start(),
    repeat(N - 1).

start() ->
    Transaction = fun(Worker) -> timer:sleep(3000), erlang:display([Worker]) end,
...
basic_test_() ->
    ...
    { "transaction",
            fun () ->
                ...
                % erlang:display([Pool|| Pool <- Pools]) ,   
